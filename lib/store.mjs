    ...
    try {
      ...
      if (stale) {
        ...
        try { recoveryFd = fs.openSync(recoveryPath, 'wx', 0o600); }
          ...
          if (process.platform === 'win32' && ['EPERM', 'EACCES', 'EBUSY'].includes(guardError.code)) {
            const remaining = until - Date.now();
            if (remaining <= 0) throw guardError;
            Atomics.wait(sleepBuffer, 0, 0, Math.min(25, remaining));
            continue;
          }
        ...
        if (recoveryFd !== undefined) {
          try {
            ...
            try { fs.unlinkSync(recoveryPath); } catch (cleanupError) { if (cleanupError.code !== 'ENOENT') throw cleanupError; }
          }
          continue;
        }
      }
