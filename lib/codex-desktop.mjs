  ...
  return new Promise((resolve, reject) => {
    ...
    function fail(error) {
      if (mutating && submitted) {
        ...
        error.message += ' Delivery is uncertain; inspect the paired task before retrying.';
      }
      finish(error);
    }
...
/**
 * Send a native visible prompt to the exact Codex task paired by the user.
...
export async function sendToCodex({ pipePath, threadId, message, turnId, callId }) {
  requiredText(threadId, 'Paired Codex task id');
  ...
  if (!result || typeof result.success !== 'boolean' || !Array.isArray(result.contentItems)) {
    const error = new Error('Codex Desktop returned an unsupported result. Inspect the paired task before retrying.');
    error.deliveryUnknown = true;
    throw error;
  }
  if (!result.success) {
